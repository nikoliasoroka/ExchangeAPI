...
internal class IdentityService : IIdentityService
{
    ...
    public async Task<IResult<string>> CreateUserAsync(string userName, string password)
    {
        ...
        return result.Succeeded
            ? Result<string>.CreateSuccess(user.Id)
            : Result<string>.CreateFailed(result.Errors.Select(e => e.Description).FirstOrDefault()!);
    }
    ...
    public async Task<IResult> DeleteUserAsync(string userId)
    {
        ...
        return user != null ? await DeleteUserAsync(user) : Result.CreateSuccess();
    }

    public async Task<IResult> DeleteUserAsync(AppUser user)
    {
        var result = await _userManager.DeleteAsync(user);

        return result.Succeeded
            ? Result.CreateSuccess()
            : Result.CreateFailed(result.Errors.Select(e => e.Description).FirstOrDefault()!);
    }
}